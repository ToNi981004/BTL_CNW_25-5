...
namespace Website_BHDT_BTL_CNWEB_.Controllers
{
    public class MyController : Controller
    {
        ...
        public ActionResult Shop(string Hang)
        {
            ...
            {
                var model = db.SanPhams.SqlQuery("select*from SanPham where Hang=N'"+ Hang + "'").ToList();
                return View(model);
        ...
        {
            var model = db.SanPhams.SqlQuery("select*from SanPham where TenSanPham like N'%" + TenSanPham + "%'").ToList();
            return View(model);
        }
